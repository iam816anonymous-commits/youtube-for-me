...
app.get('/health', (req, res) => {
  sendResponse(res, 200, true, { status: 'healthy', service: 'roadmap-service' });
});
...
app.get('/api/v1/roadmaps', async (req, res) => {
  try {
    ...
    sendResponse(res, 200, true, tenantMockData);
  }
});
...
app.post('/api/v1/roadmaps', async (req, res) => {
  ...
  try {
    ...
    sendResponse(res, 201, true, newRoadmap);
  }
});
...
const server = app.listen(PORT, () => {
  log('INFO', `Roadmap Service initialized on port ${PORT}`);
});

const gracefulShutdown = (signal) => {
  log('INFO', `Received ${signal}. Starting graceful shutdown of Roadmap Service...`);
  server.close(async () => {
    log('INFO', 'Roadmap Service HTTP server closed. Draining database connection pool...');
    try {
      await pool.end();
